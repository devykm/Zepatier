...
@implementation BridionInfoControl
...
-(void)showInfo:(id)sender
{
	if(!isLock) {
		...
		if(self.controller == nil) {
			self.controller = [[BridionInfoControlController alloc] init:infoImg];
		}
...
-(void)showView:(UIView*)view fromRect:(CGRect)rect inView:(UIView*)inView
{
	...
	float x = inViewRect.origin.x + rect.size.width + 15;
	float y = inViewRect.origin.y - rect.size.height - view.frame.size.height - 15;
	CGPoint pointView;
    if(self.window.rootViewController.presentedViewController != nil) {
