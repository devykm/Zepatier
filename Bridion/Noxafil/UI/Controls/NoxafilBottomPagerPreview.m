...
@implementation NoxafilBottomPagerPreview
...
-(void) moveToPoint:(CGPoint)point andInitItems:(NoxafilChapterData*)data
{
    ...
    for (int i=0; i<[data.pages count]; i++) {
		NoxafilPageData *pdata = (NoxafilPageData*)[data.pages objectAtIndex:i];
        if(pdata.isLobi) {
            continue;
        }
...
-(void) btnPageOpenClick:(id)sender
{
    ...
    [_delegate invanzBottomPagerPreviewSelected:self.currentChapterData.number page:btn.tag];
}
