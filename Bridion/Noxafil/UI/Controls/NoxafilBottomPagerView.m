...
@implementation NoxafilBottomPagerView
...
-(void) show
{
    ...
    isAnimating = YES;
    [self.delegate invanzBottomPagerPreviewOpen];
    [self setHidden:NO];
...
-(void) hide
{
    ...
    isAnimating = YES;
    [self.delegate invanzBottomPagerPreviewClose];
    [UIView animateWithDuration:0.4 animations:^(void){
        [self.barContainer setY:self.bounds.size.height];
...
- (void)initChapters:(NSMutableArray*)chapters
{
    ...
    for (int i=0; i<[chapters count]; i++) {
        NoxafilChapterData *data = (NoxafilChapterData*)[chapters objectAtIndex:i];
        ...
        [btn setBackgroundImage:[[UIImage imageNamed:@"btn_pager_invanz.png"] stretchableImageWithLeftCapWidth:20 topCapHeight:0] forState:UIControlStateNormal];
        [btn setBackgroundImage:[[UIImage imageNamed:@"btn_pager_invanz_s.png"] stretchableImageWithLeftCapWidth:20 topCapHeight:0] forState:UIControlStateSelected];
        [btn setBackgroundImage:[[UIImage imageNamed:@"btn_pager_invanz_s.png"] stretchableImageWithLeftCapWidth:20 topCapHeight:0] forState:UIControlStateSelected | UIControlStateHighlighted];
        ...
        btn.titleLabel.font = [UIFont fontWithName:@"Helvetica CE" size:16];
...
-(void) showPreview:(int)tag
{
    ...
    NoxafilChapterData *chapter = [self.chaptersData objectAtIndex:tag];
    float xPos = box.center.x - (previewW/2);
    xPos = MIN(xPos, self.bounds.size.width - previewW - 10);
    xPos = MAX(xPos, 10);
    if(self.previewView == nil) {
        self.previewView = [[NoxafilBottomPagerPreview alloc] initWithFrame:CGRectMake(xPos - 30.0, 0, previewW, 660)];
...
#pragma --------------
#pragma InvanzBottomPagerPreviewDelegate

-(void) invanzBottomPagerPreviewSelected:(int)chapter page:(int)page
{
    [self.delegate invanzBottomPagerPreviewSelected:chapter page:page];
