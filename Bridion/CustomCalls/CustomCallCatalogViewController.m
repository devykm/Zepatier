...
-(void) openCustomCall:(UIButton*)sender{
    NSLog(@"Selected button tag is %ld", (long)sender.tag);
...
-(void) btnClick:(UITapGestureRecognizer*)sender
{
    ...
    CustomCallsView *btn = (CustomCallsView*)sender.view;
    if ([btn isKindOfClass:[CustomCallsPageDataView class]]){
        CustomCallsPageDataView *pageView = (CustomCallsPageDataView*)btn;
        switch (appId) {
            case 5:
                [[ArcViewController getInstance]setCustomCallsFromPage:pageView.pages page:pageView.page];
        ...
        switch (appId) {
            case 5:{
                ...
                NSString *username = [Test activeUser].userName;
                [self.customCallManager sendCustomCallsStatistics:prodID userID:username];
                [[ArcViewController getInstance] setCustomCallsScrollView:btn.pages appID:appId];
